...
- **Phase 3 遗留 gap**：精剪 review_enhanced 的整句删除底稿读 `semantic_deep_analysis.json`，不读用户在粗剪页手改后的 `delete_segments_roughcut.json`——用户粗剪手改不会自动叠进精剪页。若要修，是改 generate_review_enhanced.js 叠加粗剪导出。
