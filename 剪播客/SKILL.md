...
#### 1.3 粗剪（段落级分析 + 句子级审查）→ 委托 /podcastcut-粗剪

本阶段已抽成独立单元。调用 **`/podcastcut-粗剪`**（从 `output/.current_project` 定位项目），它完成：

- 段落级 AI 内容分析（6 类删除：录前准备/技术调试/跑题闲聊/隐私/重复内容/制作讨论 + 质量优化 `suggest_delete`）→ `2_分析/semantic_deep_analysis.json`
- 生成**句子级**审查页 `review_roughcut.html`（含半句删除 + 句子序号）并打开；用户审查/编辑后导出 **`delete_segments_roughcut.json`**
- 写 `project.json`（`roughcut=awaiting_review`，用户确认后置 `approved`）

> 方法论详见 `基础剪辑规则/10-内容分析方法论.md`；激进度/类型开关由 `用户偏好/<user>/` 决定。
> 🔴 导出命名 `delete_segments_roughcut.json`，与精剪 `delete_segments_edited.json` **分开**避免撞名。

---

#### 1.4–1.5 精剪（词/句级分析 + AI 自审查 + 词级审查）→ 委托 /podcastcut-精剪

本阶段（原 1.4 精剪分析 + 1.5 AI 自审查）已抽成独立单元。调用 **`/podcastcut-精剪`**（从 `output/.current_project` 定位项目），它完成：

- **规则层** `run_fine_analysis.js` → `fine_analysis_rules.json`（句首填充词/静音/卡顿/句内重复/连续填充/重启信号，高 recall）
- **LLM 层**（Claude，按 `基础剪辑规则/LLM精剪prompt模板.md` 逐句校对）→ `fine_analysis_llm.json`（重说纠正★/残句/重复句/录制讨论）
- **合并 + 边界精修** `merge_llm_fine.js` → `fine_analysis.json`；`refine_fine_analysis.js`（波形 onset 精修紧密边界）
- **AI 自审查** 独立 prompt 查漏补缺 → `review_agent_catches.json`，`confidence≥0.7` 合并入 `fine_analysis.json`
- 生成**词级**审查页 `review_enhanced.html`（陷阱42 修复 + 半句删除 char 级精度）并打开；用户审查/编辑后导出 **`delete_segments_edited.json`**
- 写 `project.json`（`fine=awaiting_review`，用户确认后置 `approved`）

> 基础规则见 `基础剪辑规则/1-9`，用户覆盖见 `用户偏好/<user>/editing_rules/`。
> 🔴 出片用的最终清单是精剪导出的 `delete_segments_edited.json`（执行单元默认吃它）。

---

### 阶段 2: 人工审核

> **审查页与导出已并入上面的「粗剪 / 精剪」单元**：
> - 粗剪开 `review_roughcut.html`（句子级）→ 导出 `delete_segments_roughcut.json`
> - 精剪开 `review_enhanced.html`（词级）→ 导出 `delete_segments_edited.json`
>
> 两步各自暂停等用户审核（manifest `awaiting_review → approved`），对应未来网页 stepper 的"做完 → user check → complete"。下面 2.3 / 2.4 是审查导出后的**反馈学习 / 评估指标**（跨单元的可选闭环），暂留在编排器中。
