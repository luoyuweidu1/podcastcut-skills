...
**问题**：review.html 的 `exportDeleteSegments()` 只对 gap < 300ms 做 onset pullback（前移 50ms），gap ≥ 300ms 时不处理。导致 S13 "对"（gap 470ms）、S14 "嗯"（gap 280ms→已处理但类似问题）删除后仍有残音。
...
**修复**：在 `getSkipRanges()` 和 `exportDeleteSegments()` 中，合并跳过范围后增加"打孔"步骤——遍历所有非删除句，将其时间范围从跳过范围中挖除。已同步修复模板文件 `templates/review.html`。**已修复**。
...
**修复**：`generate_review.js` 增加 `incomingSilences` 字段，将 silence 编辑同时传给下一个非删除句。`review.html` 模板在句首渲染 `⏸ -Xs` 标记（黄色虚线边框，可点击联动 toggleFineEdit）。**已修复**。
...
### 陷阱 37: review.html 注入状态覆盖用户编辑

**现象**：用户在审查页修改后刷新页面，所有修改丢失。

**根因**：`generate_review.js` 注入的 `_injectedState` 代码在每次页面加载时无条件覆盖 localStorage，即使 localStorage 中有更新的用户编辑。
...
fine_analysis.json (更新后)
     ↓
generate_review.js / cut_audio.py
```
